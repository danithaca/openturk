...
class MTAnalysisApp(JythonDrupalApp):
  ...
  def _init_settings(self):
    ...
    settings = self.readEncryptedSettingsField(task_encrypted_settings);

    # the main question used to compute fleiss and final results
    self.main_question = settings.get('main_question')

    # how much greater the majority results has to be in order to return a definite results
    self.majority_advantage = settings.get('analysis_results_majority_advantage')
    ...
    self.minimum_requirement = settings.get('analysis_results_minimum_requirement')
    self.minimum_requirement = int(self.minimum_requirement) if self.minimum_requirement != None else 0

    self.ignored_answers = settings.get('analysis_results_ignored_answers')
  ...
  def compute_all(self):
    self.results['total_hits'] = self.queryValue("SELECT COUNT(hit_id) FROM {mt_task_hit} WHERE task_id=?", self.task_id)
    if self.results['total_hits'] == 0:
      return Result(False, 'Total HITs is 0, please load HITs first before computing results.')


    row = self.query("SELECT MIN(h.max_assignments) m1, MAX(h.max_assignments) m2, SUM(h.max_assignments) m3\
    ...
    if len(assignments) == 0:
      return Result(False, 'Total assignments is 0, please load assignments first before computing results.')
    ...
    rows = self.query("SELECT h.hit_id, h.annotation_id FROM {mt_hit} h INNER JOIN {mt_task_hit} t ON h.hit_id=t.hit_id WHERE t.task_id=?", self.task_id)
    for row in rows:
